...
type ApiGatewayEvent = { authorizationToken: string | undefined; methodArn: any };
...
export const authorizer: Authorizer = (event, requiredRole) => {
    try {
        ...
        if (!decodedData) return MessageUtil.error(MessageUtil.errorCode.unauthorized, 10000, "Unauthorized");
        if (authStrategies[requiredRole](decodedData))
            return generatePolicy(decodedData.id, "Allow", event.methodArn, decodedData);
        return generatePolicy(null, "Deny", event.methodArn, { error: "unauthorized", action: null });
    } catch (error) {
